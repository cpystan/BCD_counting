...
class LearnedPositionalEncoding(nn.Module):
    def __init__(self, max_position_embeddings, embedding_dim, seq_length):
        ...
        self.pe = nn.Embedding(max_position_embeddings+2, embedding_dim)
        self.seq_length = seq_length+2
...
class VisionTransformer_Encoder(nn.Module):
    def __init__(
        ...
        self.head = nn.Sequential(default_conv(3,64,3),ResBlock(default_conv,64,3) , ResBlock(default_conv,64,3) , ResBlock(default_conv,64,3),default_conv(64,3,3))
        ...
        self.output = nn.ModuleList( [nn.Sequential(
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(embedding_dim, 1) )for _ in [0,1]])
        '''
    ...
    def forward(self, x, con=False):
        ...
        x = torch.cat((self.cls_token1.repeat(x.shape[0], 1, 1), x), dim=1)
        ...
        x0 = self.output[0](x[:,-1,:])
        x1 = self.output[1](x[:, -2, :])
        return [x0 , x1]
